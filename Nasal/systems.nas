...
###############
    check_warning:func{
        ...
        if(pwr<5){
            ...
            if(getprop("instrumentation/doors/passenger/position-norm")>0){
                cbndoor=1;
                master=1;
            }
        }
...
setprop("instrumentation/zkv1000/pfd/status",1);
...
#This is a very simple fake electric system for the lights
var battery=getprop("/controls/electric/battery-switch");
if(battery==1){
    setprop("/systems/electrical/outputs/nav-lights-norm", getprop("/controls/lighting/nav-lights"));
    if(getprop("/controls/lighting/strobe-lights")==1){
    setprop("/systems/electrical/outputs/strobe-norm", getprop("/sim/model/lights/sbc1/state"));
    setprop("/systems/electrical/outputs/beacon-norm", getprop("/sim/model/lights/beacon/state"));
    }
    }else{
    setprop("/systems/electrical/outputs/nav-light-norm", 0);
    setprop("/systems/electrical/outputs/strobe-norm", 0);
    setprop("/systems/electrical/outputs/beacon-norm", 0);
    }
    
#engines on off
var thrL=getprop("/controls/engines/engine/throttle");
var thrR=getprop("/controls/engines/engine[1]/throttle");
var masterL=getprop("/controls/engines/engine/master");
var masterR=getprop("/controls/engines/engine[1]/master");
if(masterL == 1){
setprop("/controls/engines/engine/throttle-internal", thrL);
}else{
setprop("/controls/engines/engine/throttle-internal", 0);
}
if(masterR == 1){
setprop("/controls/engines/engine[1]/throttle-internal", thrR);
}else{
setprop("/controls/engines/engine[1]/throttle-internal", 0);
}

    #from dhc6, thrust reverse -> as we are now using jet engines probably not needed anymore
...
#direct wheel thruster -> The DWT (direct wheel thrusting) system consists of an electric engines positioned at the nose gear and directly connected to the nose wheels. So obviouls it only works
#when the aircraft is on the ground (otherwise wheels have no effect). A security system stops the DWT when the engines are running to prevent them from damaging the DWT.
#When not running the DWT gains power back through acting as a generator (not implemented yet).
    var wow =getprop("/gear/gear[0]/wow");
    var eng1 =getprop("/controls/engines/engine/master");
    var eng2 =getprop("/engines/engines/engine[1]/master");
    var wts =getprop("/controls/electric/battery-switch");
    if(wow==1){
                if(wts==1){
                    var thrust=getprop("/controls/engines/engine[0]/throttle");
                    setprop("/sim/model/pushback/force",thrust or 0);
                    var rvrs0=getprop("/controls/engines/engine[0]/reverser");
                    var rvrs1=getprop("/controls/engines/engine[1]/reverser");
                    if(rvrs0==1 and rvrs1==1){
                        setprop("/sim/model/pushback/force-rvs",thrust or 0);
